...
RUSTFLAGS="-C target-cpu=native" && cd python_kyber && maturin develop --features "${VER}" --release
cd ..
RUSTFLAGS="-C target-cpu=native" && cd check_bp && maturin develop --features "${VER}" --release
cd ..
